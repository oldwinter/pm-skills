...
这是 `code-review` 的中文 runtime 入口。

中文产品管理请求命中本 skill 时，默认执行 `correctness` 维度；只有用户明确指定时才启用 `performance` 或 `security` 子维度。先说明评审范围、比较基线和所选维度，再沿跨边界的参与者关系构造可达的违规执行，逐项核验约束、证据、影响与反驳，最后用简体中文报告可支持的发现和未决未知项。输出保留 PM framework 名称、维度名、命令、路径、文件名、引用、代码、配置键及报告字段原样；不要把静态评审结果表述为已确认漏洞或实测性能回归。
